...
module.exports = async (client, message, command) => {
  if (!command.anyClientPermissions || !message.guild || !Array.isArray(command.anyClientPermissions) || !message.guild)
    return true;
  const member = message.member;
  if (
    command.anyClientPermissions.some(permission => message.guild.members.me.permissions.toArray().includes(permission))
  )
    return true;
  else {
